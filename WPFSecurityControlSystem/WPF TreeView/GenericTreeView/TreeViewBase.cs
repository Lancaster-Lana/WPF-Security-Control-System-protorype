...
namespace Controls.WpfUI.GenericTreeView
{
    ...
    public abstract partial class TreeViewBase<T> : UserControl where T : class
    {
        ...
        /// <summary>
        /// Refresh subtree after aperations: add, remove, update tree items
        /// </summary>
        /// <param name="parentItem">The top item to be refreshed</param>
        /// <param name="navigationItem">The item to be selected after update</param>
        public virtual void RefreshNode(T parentItem, T navigationItem)
        {
            if (parentItem == null)
                return;

            TreeViewItem refreshNode = TryFindNode(parentItem);
            ...
            TreeViewItem navigateNode = TryFindNode(navigationItem);

            //Refresh after add
            if (navigateNode == null)
            {
                //If not found navigation node - create TreeNode and navigate it
                navigateNode = CreateItemNode(navigationItem, refreshNode.Items, currentLayout);
            }
            else
            {
                var childItems = GetChildItems(parentItem);

                //TODO: Refresh child nodes after remove, add        
                monitor.RemoveNodes(refreshNode.Items);
                refreshNode.Items.Clear();

                foreach (T childItem in childItems)
                    CreateItemNode(childItem, refreshNode.Items, currentLayout);
                refreshNode.Items.Refresh();

                /*
                if (childItems.Count != refreshNode.Items.Count) //So, there are items to be removed
                {
                    var nodesToBeRemoved = (from TreeViewItem node in refreshNode.Items
                                           where !childItems.Contains(node.Header)
                                           select node).ToList();
                    foreach (TreeViewItem node in nodesToBeRemoved)
                        refreshNode.Items.Remove(node);

                    refreshNode.Items.Refresh();
                }
                //if (!childItems.Contains(navigationItem)) // navigationItem had been removed - child items collection should be updated
                //{
                //    refreshNode.Items.Remove(navigationItem);
                //    refreshNode.Items.Refresh();
                //}
                else //Simple refresh : refresh itself after edit
                {
                    var updatingItemIndex = childItems.IndexOf(navigationItem);
                    ((TreeViewItem)refreshNode.Items[updatingItemIndex]).Header = navigationItem; //Update header information
                    refreshNode.Items.Refresh();
                    //     var newName =  Convert.ToString(parent.Name); //Name property should present
                    //     refreshNode.Header = navigationItem;
                    //     refreshNode.UpdateLayout();
                    //     //Items.IndexOf(parentItem);
        ...
        protected virtual void OnNodeExpanded(TreeViewItem treeNode)
        {
            ...
            if (treeNode.Items.Count == 0 && childItems != null)
            {
                foreach (T childItem in childItems)
                {
        ...
        protected internal TreeViewItem CreateItemNode(T item, IList parentNodes, TreeLayout layout)
        {
            ...
            if (renderChilds || ObserveChildItems)
            {
                ...
                hasChilds = childItems != null ? childItems.Count > 0 : false;
            }
            else
            {
            ...
            if (renderChilds && childItems != null)
            {
        ...
        public TreeViewItem SelectedNode
        {
            get
            {
                return Tree.SelectedItem as TreeViewItem;
            }
        }

        public TreeViewItem GetTreeNode(T item)
        {
            return RecursiveNodeList.Where(n => n.Header == item).FirstOrDefault();
        }
        ...
        public List<T> RecursiveObjectsList
        {
            get
            {
                var list = (from node in RecursiveNodeList
                            select node.Header as T).ToList();
                return list;
            }
        }
        ...
        public abstract ObservableCollection<T> GetChildItems(T parent);
